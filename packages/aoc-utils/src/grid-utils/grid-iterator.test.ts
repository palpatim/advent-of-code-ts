...
describe("GridIterator", () => {
  test("handles simple case", () => {
    ...
    expect(result.value.value).toEqual(expectedResults.shift());
    ...
    expect(iterator.next().value.value).toEqual(expectedResults.shift());
    expect(iterator.next().value.value).toEqual(expectedResults.shift());
    expect(iterator.next().value?.value).toBeUndefined();
  ...
  test("next with argument", () => {
    const iterator = new GridIterator(getGrid());
    expect(iterator.next(2).value.value).toBe(1);
    expect(iterator.next(1).value.value).toBe(2);
  ...
  test("peek", () => {
    ...
    expect(iterator.peek()?.value).toBe(1);
  ...
  test("currentPoint", () => {
    ...
    let result = iterator.next();
    expect(result.value.point).toEqual({ row: 0, col: 0 });
    expect(result.value.value).toEqual(0);

    expect(iterator.currentPoint).toEqual({ row: 0, col: 1 });
    result = iterator.next();
    expect(result.value.point).toEqual({ row: 0, col: 1 });
    expect(result.value.value).toEqual(1);
  });
  ...
  test("GridIterableResult contains point", () => {
    ...
    expect(iterator.next().value.point).toEqual({ row: 0, col: 1 });
  });
