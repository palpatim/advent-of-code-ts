...
export class BrokenGridIterator<T> implements Iterable<T> {
  grid: Grid<T>;
  row: number;
  col: number;
  done: boolean;
  ...
  [Symbol.iterator] = (): BrokenGridIterator<T> => {
    return this;
  };
  ...
  currentResult = (): BrokenGridIteratorResult<T> | undefined => {
    if (this.done) {
      return undefined;
    }
   ...
   */
  findIndex = (pred: (v: T) => boolean): BrokenGridIteratorResult<T> => {
    while (!this.done) {
      const value = getCellAtPoint(this.currentPoint(), this.grid)!;
      if (pred(value)) {
  ...
  next = (count: number = 1): BrokenGridIteratorResult<T> => {
    if (count < 1) {
      throw new Error("Count must be greater than 0");
    }

    let i = 0;
    let result: BrokenGridIteratorResult<T>;
    for (; i < count; i++) {
      result = this._next();
    }
    return result!;
  ...
  private _next(): BrokenGridIteratorResult<T> {
