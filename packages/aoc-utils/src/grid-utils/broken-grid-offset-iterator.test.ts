import { BrokenGridOffsetIterator } from "./broken-grid-offset-iterator";
...
describe("GridOffsetIterator", () => {
  test("handles simple case", () => {
    ...
    const iterator = new BrokenGridOffsetIterator(getGrid(), getOrigin(), "S");
  ...
  test("supports for-in iteration", () => {
    const expectedResults = [0, 3, 6];
    const iter = new BrokenGridOffsetIterator(getGrid(), getOrigin(), "S");
  ...
  test("handles invalid starting point", () => {
    const iterator = new BrokenGridOffsetIterator(
      getGrid(),
      { row: -1, col: 0 },
      "S"
    );
